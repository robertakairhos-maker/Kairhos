...
export const CandidatesPool: React.FC = () => {
    ...
    return (
        <div className="flex h-full overflow-hidden bg-background-light dark:bg-background-dark">
            ...
            {/* Left Filter Sidebar */}
            <aside className="w-80 flex-shrink-0 bg-white dark:bg-[#1a212d] border-r border-slate-200 dark:border-slate-800 flex flex-col h-full overflow-y-auto custom-scrollbar">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 sticky top-0 bg-white dark:bg-[#1a212d] z-10">
                    <h2 className="text-lg font-bold text-[#111318] dark:text-white flex items-center gap-2">
                        <span className="material-symbols-outlined text-primary">filter_alt</span>
                        Filtros Avançados
                    </h2>
                    <p className="text-xs text-slate-500 mt-1">Refine sua busca no banco de talentos</p>
                </div>
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                <header className="h-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 shrink-0">
                    <div>
                        <h1 className="text-2xl font-bold text-[#111318] dark:text-white">Banco de Talentos ({filteredCandidates.length})</h1>
                        <p className="text-sm text-slate-500">
                            {selectedSkills.length > 0 
                                ? `Filtrando por ${selectedSkills.length} habilidades` 
                                : 'Visualizando todos os candidatos cadastrados'}
                        </p>
                    </div>
                </header>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {filteredCandidates.length === 0 ? (
                            ...
                            filteredCandidates.map(candidate => (
                                <div key={candidate.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm hover:shadow-md transition-all flex flex-col lg:flex-row items-start lg:items-center gap-6 group">
                                    {/* Avatar */}
                                    ...
                                    {/* Meta & Actions */}
                                    <div className="flex flex-row lg:flex-col items-center lg:items-end gap-4 w-full lg:w-auto border-t lg:border-t-0 border-slate-100 dark:border-slate-800 pt-4 lg:pt-0">
